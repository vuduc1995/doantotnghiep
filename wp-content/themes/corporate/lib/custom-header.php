...
add_action('admin_menu', 'corporate_add_settings_boxes', 11);
function corporate_add_settings_boxes() {
	global $_genesis_theme_settings_pagehook;
	
	add_meta_box('genesis-theme-settings-header', __('Header Image Settings', 'corporate'), 'corporate_theme_settings_header_box', $_genesis_theme_settings_pagehook, 'column2');
}

function corporate_theme_settings_header_box() { ?>
	...
	<p><label><?php _e('Header Image Height', 'corporate'); ?>: <input style="margin:0 5px 0 0;text-align:center;" type="text" name="<?php echo GENESIS_SETTINGS_FIELD; ?>[header_height]" value="<?php genesis_option('header_height'); ?>" size="2" />px</label></p>
<?php
}

// Add new default values for the Custom Header
add_filter('genesis_theme_settings_defaults', 'corporate_header_defaults');
function corporate_header_defaults($defaults) {

	$defaults['header_height'] = 120;
