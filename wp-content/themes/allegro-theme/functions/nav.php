...
function customized_nav_btns($page_num,$max_num_pages,$search=0) {
		...
		$args = array(
			...
			'mid_size'     	=> 2,
			'prev_next'    	=> true,
			'prev_text'    	=> '<span class="icon-text left">&#9666;</span>'.esc_html__('Previous Page','allegro-theme'),
			'next_text'    	=> esc_html__('Next Page','allegro-theme').'<span class="icon-text right">&#9656;</span>',
			'type'         	=> 'plain',
			'add_args'     	=> false,
			'add_fragment' 	=> ''
		);
	...
	if($page_num == '' && $page_num == 0){ $page_num = '1'; }
		...
		<div class="gallery-navi">
			...
			<?php if($page_num > 1) { ?>
						...
						<!--<a href="<?php if ($page_num > 1) { $new_page = $page_num - 1;} else {$new_page = 1;} echo custom_nav_btn_links($search, $new_page); ?>" class="prev"><?php printf ( esc_html__('Previous','allegro-theme'));?></a>-->
			<?php } else { ?>
